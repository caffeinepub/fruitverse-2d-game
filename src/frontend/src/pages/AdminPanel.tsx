...
import {
  ...
  useBanUser,
  useGetAllAudioMetadata,
  useGetAllUsers,
  useGetTopPlayers,
  useUnbanUser,
  useUploadAudioFromUrl,
...
const audioFileDescriptions: Record<
  string,
  { name: string; description: string }
> = {
  fruitDrop: {
    name: "Meyve Düşüşü",
    description: "Meyveler düştüğünde çalan ses",
  },
  fruitMerge: {
    name: "Meyve Birleşmesi",
    description: "Meyveler birleştiğinde çalan ses",
  },
  bombExplode: {
    name: "Bomba Patlaması",
    description: "Bomba güçlendiricisi kullanıldığında çalan ses",
  },
  alignmentPing: {
    name: "Hizalama Sesi",
    description: "Hizalama güçlendiricisi kullanıldığında çalan ses",
  },
  scoreUp: { name: "Skor Artışı", description: "Skor arttığında çalan ses" },
  gameStart: {
    name: "Oyun Başlangıcı",
    description: "Oyun başladığında çalan ses",
  },
  gameOver: { name: "Oyun Sonu", description: "Oyun bittiğinde çalan ses" },
  levelUp: {
    name: "Seviye Atlama",
    description: "Seviye atlandığında çalan ses",
  },
  boosterEarned: {
    name: "Güçlendirici Kazanıldı",
    description: "Güçlendirici kazanıldığında çalan ses",
  },
  backgroundLoop: {
    name: "Arka Plan Müziği",
    description: "Oyun sırasında sürekli çalan müzik",
  },
};
...
export default function AdminPanel({ onExit }: AdminPanelProps) {
  ...
  const handleBanAction = (targetUsername: string, isBanned: boolean) => {
    setBanTarget({ username: targetUsername, isBanned });
  ...
  const confirmBanAction = async () => {
    ...
    try {
      if (banTarget.isBanned) {
        await unbanUserMutation.mutateAsync({ username: banTarget.username });
        toast.success(`Yasak kaldırıldı: ${banTarget.username}`);
      } else {
        await banUserMutation.mutateAsync({ username: banTarget.username });
        toast.success(`Kullanıcı yasaklandı: ${banTarget.username}`);
      }
  ...
  const renderAudioUploadSection = (
    categoryKey: keyof typeof audioCategories,
  ) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Tüm backend verilerini yönetin
              </p>
            </div>
          </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      MP3 dosyalarını URL üzerinden yükleyin ve blob storage'da
                      saklayın
                    </CardDescription>
                  </div>
                  <Button
                ...
                {audioLoading ? (
                  ...
                  <Tabs defaultValue="gameEffects" className="space-y-4">
                    <TabsList className="bg-gray-100">
                      <TabsTrigger value="gameEffects">
                        Oyun Efektleri
                      </TabsTrigger>
                      <TabsTrigger value="gameStates">
                        Oyun Durumları
                      </TabsTrigger>
                      <TabsTrigger value="backgroundMusic">
                        Arka Plan Müziği
                      </TabsTrigger>
                    </TabsList>
                ...
                {usersLoading ? (
                  ...
                  <ScrollArea className="h-[500px]">
                    <Table data-ocid="users.table">
                      ...
                      <TableBody>
                        {allUsers.map(([uname, profile], index) => (
                          <TableRow
                            key={uname}
                          ...
                          >
                            ...
                            <TableCell className="text-right font-mono">
                              {Number(profile.highScore).toLocaleString()}
                            </TableCell>
                            <TableCell className="text-center text-sm text-gray-600">
                              💣{Number(profile.boosters.bombCount)} 🔄
                              {Number(profile.boosters.nextFruitChangeCount)} 📏
                              {Number(profile.boosters.alignmentCount)}
                            </TableCell>
                            ...
                            <TableCell>
                              <Button
                                variant={profile.isBanned ? "outline" : "ghost"}
                                size="sm"
                                onClick={() =>
                                  handleBanAction(uname, profile.isBanned)
                                }
                                className={
                                  profile.isBanned
                              ...
                              >
                                {profile.isBanned ? "Yasağı Kaldır" : "Yasakla"}
                              </Button>
                            </TableCell>
                          </TableRow>
                        ))}
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Aylık sıralamalar ve oyuncu skorları
                    </CardDescription>
                  </div>
                  <div className="flex gap-2">
                    <Button
                          ...
                          <TableRow key={player.username}>
                            ...
                            <TableCell>
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleDeleteUser(player.username)
                                }
                              >
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Backend sağlık kontrolü ve izleme
                    </CardDescription>
                  </div>
                  <Button
                    variant="outline"
      ...
      <AlertDialog open={resetDialogOpen} onOpenChange={setResetDialogOpen}>
        <AlertDialogContent className="bg-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Aylık lider tablosunu sıfırlamak istediğinizden emin misiniz? Tüm
              skorlar silinecek.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={banDialogOpen} onOpenChange={setBanDialogOpen}>
        <AlertDialogContent className="bg-white" data-ocid="users.dialog">
          <AlertDialogHeader>
            <AlertDialogTitle>
              {banTarget?.isBanned ? "Yasağı Kaldır" : "Kullanıcıyı Yasakla"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {banTarget?.isBanned
                ? `"${banTarget.username}" kullanıcısının yasağını kaldırmak istediğinizden emin misiniz?`
                : `"${banTarget?.username}" kullanıcısını yasaklamak istediğinizden emin misiniz?`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {banTarget?.isBanned ? "Yasağı Kaldır" : "Yasakla"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
