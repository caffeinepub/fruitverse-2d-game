...
export default function HealthMonitor() {
  ...
  const { data: healthStatus, isLoading: healthLoading } = useBackendHealthCheck();
  const [lastCheckTime, setLastCheckTime] = useState<string>('');

  useEffect(() => {
    if (healthStatus?.timestamp) {
      const date = new Date(healthStatus.timestamp);
      setLastCheckTime(date.toLocaleTimeString(language, { 
        hour: '2-digit', 
        minute: '2-digit', 
        second: '2-digit' 
      }));
    ...
    <Card className="w-full max-w-md bg-white/90 backdrop-blur-sm">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  <span className="ml-1">{t(`health.status.${healthStatus?.status || 'unknown'}`)}</span>
                </Badge>
              </TooltipTrigger>
              <TooltipContent>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-medium">{healthLoading ? t('health.checking') : lastCheckTime}</span>
        </div>
        ...
        <div className="text-xs text-muted-foreground">
          ...
          {healthStatus?.status === 'disconnected' && (
            <p className="text-yellow-600">{t('health.message.disconnected')}</p>
          )}
        </div>
      </CardContent>
    </Card>
